...
namespace channelInspection.Schemas
{
    ...
    public class 로그테이블 : Data.BaseTable
    {
        ...
        public async Task<int> InsertAsync(로그정보 정보)
        {
            ...
            catch (Exception ex) { Util.DebugException(ex, 3, "로그저장"); }
            return 0;
        }
        ...
        public int 자료정리(int 일수)
        {
            ...
            //String Sql = $@"DELETE FROM logs WHERE ltime < DATE('{Util.FormatDate(일자, "{0:yyyy-MM-dd}")}')";
            String Sql = $@"DELETE FROM logs WHERE ltime < DATE('{일자:yyyy-MM-dd}')";
            try
            {
                int AffectedRows = 0;
